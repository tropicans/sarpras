...
import { users, sessions, assets, assetAvailability, assetClosures, auditLogs } from "./schema";
...
test("Phase 2 Secure Administration & Asset Setup - DB & Logic Tests", async (t) => {
  ...
  await db.delete(assetAvailability).where(like(assetAvailability.openTime, `${testPrefix}%`));
  await db.delete(assetClosures).where(eq(assetClosures.id, "00000000-0000-0000-0000-000000000000")); // dummy check
  ...
  await t.test("D-14: Users Status and reset password columns", async () => {
    ...
    await db.update(users).set({ status: "inactive" }).where(eq(users.id, `${testPrefix}uid-1`));
    const updatedUser = await db.select().from(users).where(eq(users.id, `${testPrefix}uid-1`));
    assert.strictEqual(updatedUser[0].status, "inactive");
  });

  await t.test("D-13 & D-15 & D-16: User Deactivation and Session Revocation", async () => {
    const targetUserId = `${testPrefix}uid-target`;
    const actorUserId = `${testPrefix}uid-actor`;

    // Setup actor and target users
    await db.insert(users).values([
      { id: actorUserId, name: "Actor Admin", email: `${testPrefix}actor@example.com`, role: "admin" },
      { id: targetUserId, name: "Target Operator", email: `${testPrefix}target@example.com`, role: "operator", status: "active" }
    ]);
    ...
    const activeSessionsBefore = await db.select().from(sessions).where(eq(sessions.userId, targetUserId));
    ...
    await db.insert(auditLogs).values({
      ...
      metadata: { deactivatedBy: actorUserId, timestamp: new Date().toISOString() },
    });

    // Assertions
    const targetUserInDb = await db.select().from(users).where(eq(users.id, targetUserId));
    assert.strictEqual(targetUserInDb[0].status, "inactive");

    const activeSessionsAfter = await db.select().from(sessions).where(eq(sessions.userId, targetUserId));
    assert.strictEqual(activeSessionsAfter.length, 0); // sessions must be deleted!

    const logs = await db.select().from(auditLogs).where(eq(auditLogs.entityId, targetUserId));
  ...
  await t.test("D-09: Asset Availability Table insertions", async () => {
    ...
    // Insert availability
    const availResult = await db.insert(assetAvailability).values({
      assetId,
      dayOfWeek: 1, // Monday
    ...
    // Query availability
    const avails = await db.select().from(assetAvailability).where(eq(assetAvailability.assetId, assetId));
  ...
  await t.test("D-10: Asset Closures Table insertions", async () => {
    ...
    // Insert closure
    const closureResult = await db.insert(assetClosures).values({
      assetId,
      date: closureDate,
    }).returning();
    ...
    // Query closures
    const closures = await db.select().from(assetClosures).where(eq(assetClosures.assetId, assetId));
    assert.strictEqual(closures.length, 1);
    assert.strictEqual(closures[0].date.getTime(), closureDate.getTime());
  });

  await t.test("D-11 & D-12: Asset archiving and Timezone validation", async () => {
    ...
    const archivedAsset = await db.select().from(assets).where(eq(assets.id, assetId));
