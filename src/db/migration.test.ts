...
test("Phase 1 Canonical Data & Migration Tests", async (t) => {
  ...
  await t.test("DATA-04: Timezone Interpretation & Normalization", () => {
    ...
    const formatted = formatInTimeZone(interpretedDate, "Asia/Jakarta", "yyyy-MM-dd HH:mm:ss");
    assert.strictEqual(formatted, "2026-08-15 09:00:00");
  });

  await t.test("DATA-01 & DATA-03: Idempotency and Database Integrity", async () => {
    // 1. Insert test asset
    ...
    const assetInDb = await db.select().from(assets).where(eq(assets.legacyId, testAssetLegacyId));
    ...
    await assert.rejects(
      ...
      (err: any) => err instanceof Error && err.message.includes("Failed query")
    );

    // 2. Insert test booking linked to asset
    const testBookingLegacyId = `${testPrefix}booking-1`;
    const startLocal = toDate("2026-08-15T09:00:00", { timeZone: "Asia/Jakarta" });
    const endLocal = toDate("2026-08-15T12:00:00", { timeZone: "Asia/Jakarta" });
    ...
    const bookingInDb = await db.select().from(bookings).where(eq(bookings.legacyId, testBookingLegacyId));
