...
import { assets, users, accounts, bookings, auditLogs } from "./schema";
...
async function main() {
  ...
  if (!fs.existsSync(legacyDataDir)) {
    console.error(`Error: Directory 'legacy-data' does not exist in workspace root.`);
    process.exit(1);
  }
  ...
  const loadJSON = (filePath: string): any[] => {
    if (!fs.existsSync(filePath)) {
      console.warn(`Warning: File '${path.basename(filePath)}' not found. Skipping.`);
      return [];
    }
    try {
  ...
  const stats = {
    assets: { total: rawAssets.length, imported: 0, skipped: 0, rejected: 0, errors: [] as string[] },
    admins: { total: rawAdmins.length, imported: 0, skipped: 0, rejected: 0, errors: [] as string[] },
    bookings: { total: rawBookings.length, imported: 0, skipped: 0, rejected: 0, errors: [] as string[] },
  };
  ...
  for (const rawAsset of rawAssets) {
    try {
      ...
      if (!parsed.success) {
        ...
        stats.assets.errors.push(`Asset ${rawAsset.id || "unknown"}: Validation failed - ${parsed.error.message}`);
        continue;
      }
      ...
      stats.assets.rejected++;
      stats.assets.errors.push(`Asset ${rawAsset.id || "unknown"}: Exception - ${e.message}`);
    }
  }
  ...
  for (const rawAdmin of rawAdmins) {
    try {
      ...
      if (!parsed.success) {
        ...
        stats.admins.errors.push(`Admin ${rawAdmin.id || "unknown"}: Validation failed - ${parsed.error.message}`);
        continue;
      }
      ...
      stats.admins.rejected++;
      stats.admins.errors.push(`Admin ${rawAdmin.id || "unknown"}: Exception - ${e.message}`);
    }
  }
  ...
  for (const rawBooking of rawBookings) {
    try {
      ...
      if (!parsed.success) {
        ...
        stats.bookings.errors.push(`Booking ${rawBooking.id || "unknown"}: Validation failed - ${parsed.error.message}`);
        continue;
      }
      ...
      if (!canonicalAssetId) {
        stats.bookings.rejected++;
        stats.bookings.errors.push(`Booking ${bookingData.id}: Refused - Legacy asset ID '${bookingData.asset_id}' has no migrated target`);
        continue;
      }
      ...
      const startDateZoned = toDate(bookingData.start_date, { timeZone: "Asia/Jakarta" });
      const endDateZoned = toDate(bookingData.end_date, { timeZone: "Asia/Jakarta" });

      if (isNaN(startDateZoned.getTime()) || isNaN(endDateZoned.getTime())) {
        stats.bookings.rejected++;
        stats.bookings.errors.push(`Booking ${bookingData.id}: Date parsing exception - Start: ${bookingData.start_date}, End: ${bookingData.end_date}`);
        continue;
      }
      ...
      await db.transaction(async (tx) => {
        const newBooking = await tx
          ...
          .values({
            ...
            createdAt: bookingData.created_at ? toDate(bookingData.created_at, { timeZone: "Asia/Jakarta" }) : undefined,
            updatedAt: bookingData.updated_at ? toDate(bookingData.updated_at, { timeZone: "Asia/Jakarta" }) : undefined,
          })
          .returning();
      ...
      stats.bookings.rejected++;
      stats.bookings.errors.push(`Booking ${rawBooking.id || "unknown"}: Exception - ${e.message}`);
    }
  }
  ...
  console.log("| Entity            | Source Count| Imported     | Skipped      | Rejected     |");
  console.log(lineSeparator);
  console.log(
