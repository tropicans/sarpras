...
test("Facility Tag Sanitization & Fallback Resolver", async (t) => {
	await t.test(
		"sanitizeFacilities: handles dirty inputs, non-arrays, and invalid elements",
		() => {
	...
	await t.test(
		"sanitizeFacilities: case-insensitive deduplication and trimming",
		() => {
	...
	await t.test(
		"sanitizeFacilities: enforces length limits per tag and total tag caps",
		() => {
	...
	await t.test(
		"getAssetFacilities: returns custom sanitized facilities when present",
		() => {
	...
	await t.test(
		"getAssetFacilities: returns empty array by default when facilities is null, undefined, or empty",
		() => {
			assert.deepStrictEqual(
			...
			assert.deepStrictEqual(getAssetFacilities({ type: "vehicle" }), []);
			assert.deepStrictEqual(getAssetFacilities({ type: "field" }), []);
			assert.deepStrictEqual(getAssetFacilities({ type: "equipment" }), []);
		},
	);
});
