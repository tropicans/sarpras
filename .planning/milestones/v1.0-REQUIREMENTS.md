...
- [x] **ASSET-04**: Visitor can browse rooms and dormitories and filter availability by type and requested date/time without seeing another requester's personal data.

### Public Booking

- [x] **BOOK-01**: Visitor can submit a responsive, accessible room booking request with requester, organization, contact, purpose, attendance, asset, date, and time details.
- [x] **BOOK-02**: Visitor can submit a responsive, accessible dormitory booking request with the required stay dates and requester details.
- [x] **BOOK-03**: System validates required fields, dates, times, capacity, operating availability, and closures before accepting a public request.
- [x] **BOOK-04**: System returns a non-guessable reference and a clear `pending` confirmation after a valid request is submitted.
- [x] **BOOK-05**: Visitor can view the privacy-safe status of a request using its reference without gaining access to administrative data.

### Booking Decisions & Integrity

- [x] **FLOW-01**: System manages each booking through explicit `pending`, `approved`, `rejected`, and `cancelled` states.
- [x] **FLOW-02**: Authorized administrator can review pending requests with the relevant asset, schedule, requester, and conflict context.
- [x] **FLOW-03**: Authorized administrator can approve or reject a pending request, recording an explanation for rejections.
- [x] **FLOW-04**: System prevents overlapping approved reservations for the same asset during concurrent booking or approval actions.
- [x] **FLOW-05**: System rechecks authoritative availability when a booking is created or approved rather than relying solely on a client-side calendar.

### Operations & Audit

- [x] **OPS-01**: Authorized administrator can use a dashboard to view summary counts and filter bookings by status, asset type, and date range.
- [x] **OPS-02**: Authorized administrator can view an asset-centric administrative calendar showing booking and closure context.
- [x] **OPS-03**: System records an append-only audit event for each migration, booking-state decision, material booking change, and administrative asset change.
- [x] **OPS-04**: Authorized administrator can view audit history with actor, timestamp, action, and affected record context.
